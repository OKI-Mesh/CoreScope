...
func OpenStoreWithInterval(dbPath string, sampleIntervalSec int) (*Store, error) {
	...
	state, _, err := database.MigrationStatus(db)
	if err != nil {
		db.Close()
	...
	switch state {
	case database.StateFresh, database.StateMigrated:
		if err := database.RunMigrations(db); err != nil {
			db.Close()
			return nil, fmt.Errorf("run migrations: %w", err)
		}
	case database.StateUnstamped:
		if err := database.DetectAndStampSchema(db); err != nil {
			db.Close()
			return nil, fmt.Errorf("auto-stamping: %w", err)
		}

		version, err := database.CurrentSchemaVersion(db)
		if err != nil {
			db.Close()
			return nil, fmt.Errorf("checking schema version after detection: %w", err)
		}

		if version == 0 {
			// Detection found nothing to stamp — this is functionally a
			// fresh replay, not a gap-fill. Plain RunMigrations (no
			// allow-missing) avoids goose's "no next version found"
			// error when nothing is stamped yet.
			if err := database.RunMigrations(db); err != nil {
				db.Close()
				return nil, fmt.Errorf("run migrations (full replay after detection found nothing): %w", err)
			}
		} else {
			if err := database.RunMigrationsAllowingGaps(db); err != nil {
				db.Close()
				return nil, fmt.Errorf("run migrations after auto-stamp: %w", err)
			}
		}
	}
	...
	// _migrations tracks the legacy async/one-shot migration gates
	// (BackfillPathJSONAsync, tx_last_seen_backfill_v1, etc.) — NOT
	// superseded by goose, since these are long-running background jobs
	// goose has no concept of running.
	if err := database.EnsureLegacyMigrationsTable(db); err != nil {
		db.Close()
	...
	// obs_observer_ts_idx_v1 async index build — superseded by goose
	// migration 00023_obs_observer_ts_idx_v1.sql, which creates the same
	// index unconditionally via RunMigrations on every boot. Commented
	// out, not deleted, pending prod burn-in confirmation (see epic:
	// "Burn-in and delete legacy migration code").
	//
	// var idxDone int
